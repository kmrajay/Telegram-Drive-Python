...
type PreviewCacheValue = { src: string; cachedAt: number; };
...
const getCachedPreview = (key: string): string | null => {
    ...
    if (Date.now() - value.cachedAt > PREVIEW_CACHE_TTL_MS) { previewCache.delete(key); return null; }
    touchPreviewCache(key, value);
    return value.src;
};

const rememberPreview = (key: string, src: string) => touchPreviewCache(key, { src, cachedAt: Date.now() });
const forgetPreview = (key: string) => previewCache.delete(key);
...
export function PreviewModal({ file, onClose, onNext, onPrev, currentIndex, totalItems, nextFile, prevFile, activeFolderId }: PreviewModalProps) {
    ...
    useEffect(() => { setRetryCount(0); setReloadNonce(0); }, [file.id, activeFolderId]);
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                if (requestId !== latestRequestRef.current) return;
                if (path) {
                    // path is either a data: URL or a server path
                    const normalized = path.startsWith('data:') ? path : path;
                    setSrc(normalized);
    ...
    useEffect(() => {
        ...
        candidates.forEach((candidate) => {
            ...
            api.getPreview(candidate.id, activeFolderId).then((path) => {
                if (!path) return;
                rememberPreview(key, path.startsWith('data:') ? path : path);
            }).catch(() => {}).finally(() => { pendingPrefetch.delete(key); });
        });
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'l') { e.preventDefault(); onNext?.(); return; }
            if (e.key === 'ArrowLeft' || e.key.toLowerCase() === 'j') { e.preventDefault(); onPrev?.(); return; }
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="relative max-w-5xl w-full max-h-screen flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
                <button onClick={onPrev} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/60 hover:bg-black/80 rounded-full transition-colors" style={{ color: '#ffffff' }}><ChevronLeft className="w-6 h-6" /></button>
                <button onClick={onNext} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/60 hover:bg-black/80 rounded-full transition-colors" style={{ color: '#ffffff' }}><ChevronRight className="w-6 h-6" /></button>
                <button onClick={onClose} className="absolute -top-12 right-0 p-2 bg-black/60 hover:bg-black/80 rounded-full transition-colors" style={{ color: '#ffffff' }}><X className="w-6 h-6" /></button>
                ...
                {error && (
                    <div className="text-red-400 bg-white/10 p-4 rounded-lg border border-red-500/20">
                        <p className="font-bold">Preview Error</p><p className="text-sm">{error}</p>
                    </div>
                )}
                {!loading && !error && src && (
                        ...
                        {isImageFile(file.name) ? (
                            <img src={src} className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl bg-black" alt="Preview"
                                onError={() => {
                                    const key = getPreviewCacheKey(file.id, activeFolderId);
                                    forgetPreview(key);
                                    if (retryCount < 1) { setRetryCount(prev => prev + 1); setReloadNonce(prev => prev + 1); return; }
